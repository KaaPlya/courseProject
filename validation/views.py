...
def checkpayment(request):
    if request.method == "POST":
        payment_id = request.POST['QIWI_payment_id']
        order_id = request.POST['QIWI_order_id']
        QIWI_signature = request.POST['QIWI_signature']
...
def LoginUsernamevalidation(request):
    if request.method == 'POST':
        ...
        if not User.objects.filter(username=username).exists():
            return JsonResponse({
                                    'login_username_error': 'This username does not avilable please ragister first.'})  # 'status=400' write when debug
...
def handleSignup(request):
    if request.method == 'POST':
        ...
        student_detail = StudentInfo.objects.create(username=myuser, email_id=email, mobile_no=mobile, dob=dob,
                                                    address=address)
        student_detail.save()
